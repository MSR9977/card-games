...
const LAST10_RAW: [number, number, number, number][] = [
  [591, 41, 624, 70], // slot 1 (leftmost / most recent)
...
export default function Roulette() {
  ...
  useEffect(() => {
    ...
    if (rect) {
      ...
      setBallPosition(POINTER_ANGLE, BALL_INNER);
    }
  ...
  const renderChips = useCallback(
    (currentBets: Record<string, BetEntry>, last10: number[]) => {
      ...
      // ── Draw last-10 result numbers in the SVG slots ─────────────────────────
      last10.slice(0, 10).forEach((n, i) => {
        const raw = LAST10_RAW[i];
        if (!raw) return;
        ...
        ctx.fillStyle = bgCol;
        ctx.beginPath();
        ctx.roundRect(x1 + 1, y1 + 1, bw - 2, bh - 2, 3);
        ctx.fill();

        // Border glow for most-recent (slot 0)
        if (i === 0) {
          ctx.strokeStyle =
            n === 0 ? "#27c76b" : RED_NUMS.has(n) ? "#e74c3c" : "#9aa3b0";
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.roundRect(x1 + 1, y1 + 1, bw - 2, bh - 2, 3);
          ctx.stroke();
        ...
        // Number text
        const fontSize = Math.max(7, Math.min(11, bh * 0.45));
  ...
  const onTableClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      let hitR: { x1: number; y1: number; x2: number; y2: number } | null =
        null;
      ...
      if (!hitKey || !hitR) return;

      const isNumCell = /^\d+$/.test(hitKey);
      if (!isNumCell) {
        addBet(hitKey);
        return;
      }

      const num = parseInt(hitKey);
      if (num === 0) {
      ...
      const T = BORDER_T;
      const atLeft = relX < T;
      const atRight = relX > cellW - T;
      const atTop = relY < T;
      const atBottom = relY > cellH - T;
      ...
      // ── SPLIT (vertical) OR STREET ───────────────────────────────────────────
      if (atTop || atBottom) {
        const adjRow = atTop ? rowIdx - 1 : rowIdx + 1;
        if (adjRow >= 0 && adjRow < 3) {
          const nb = TABLE_ROWS[adjRow][colIdx];
          addBet("split-" + Math.min(num, nb) + "-" + Math.max(num, nb));
          return;
        } else {
          const nums = [
            TABLE_ROWS[0][colIdx],
      ...
      // ── SPLIT (horizontal) OR SPLIT-WITH-0 ───────────────────────────────────
      if (atLeft || atRight) {
        const adjCol = atLeft ? colIdx - 1 : colIdx + 1;
            ...
            <div className="table-section">
              ...
              <div className="svg-wrap">
                ...
                <div ref={tableWrapRef} className="table-inner-wrap">
                  ...
                  <div className="click-overlay" onClick={onTableClick} />
